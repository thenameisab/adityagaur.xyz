...
<Figure
  label="Figure 1"
  caption="The pipeline view: seven headline numbers, then every stage as a card carrying its own count and value. The instruction under the section heading is the product's whole posture toward its own aggregates — click any card to see deals."
  source="The repository's own offline build, which runs the shipped frontend against a local seeded database for a fictional company. Same views, same code paths, ~130 invented deals; no customer of ours appears on it."
>
  <Shot
    src="/work/crm-dashboard/pipeline.jpg"
    alt="The pipeline dashboard: KPI cards for active pipeline, win rate, past close date, new deals, deals in hold, won and lost; below them a stage snapshot grouping new, ongoing and closed deals into clickable cards"
    width={2560}
    height={1706}
  />
</Figure>
...
<Figure
  label="Figure 2"
  caption="Founder Mode, opened with Shift+F. Seven detectors as a row of counters — two of them red — and then the receipts: every amount revision with the stage the deal was in when the number moved. The line under the heading says what the view is for: pipeline integrity signals, not visible to the team."
  source="The repository's own offline build against invented deals. The detector thresholds, the stage-when-changed reconstruction and the roll-up are the production payload builder's."
>
  <Shot
    src="/work/crm-dashboard/founder-mode.jpg"
    alt="Founder Mode: counter cards for live ARR, unvalidated pipeline, amount revisions, total revised down, stage regressions, slippage deals and zombie deals, above a table of amount revisions showing original TCV, current TCV, percentage change and the stage the deal was in when the amount changed"
    width={2560}
    height={1536}
  />
</Figure>
...
<Figure
  label="Figure 4"
  caption="Click one, from the Demo Done card in Figure 1. The count is in the title, so the drill visibly reconciles against the number that opened it; the deal name is a link out to the CRM record, which is click two and the end of this dashboard's authority."
  source="The repository's own offline build. Accounts, owners and notes are invented; the columns and the reconciliation are the shipped modal's."
>
  <Shot
    src="/work/crm-dashboard/drilldown.jpg"
    alt="A modal titled 'Demo Done · 16 deals' listing each deal with its account, owner, days in stage, total contract value and last note, with the longest-stalled deals at the top"
    width={1800}
    height={1338}
  />
</Figure>
...
<Figure
  label="Figure 5"
  caption="The v2 architecture. Read it as two paths that meet at the database: a write path that runs three times a day and is allowed to fail one source at a time, and a read path that runs on every page load and is not allowed to serve a failure. The database between them is the whole point of the split — before it, a bad fetch meant no page."
  scroll
>
  <Arch
    label="Architecture: a scheduled write path and a per-load read path, decoupled by a Postgres snapshot table, with an access wall in front of the read path"
    columns={[
      { label: "Sources", nodes: [
        { id: "crm", label: "CRM", sub: "deals · timelines", kind: "muted" },
        { id: "mkt", label: "Marketing APIs", sub: "leads · sessions", kind: "muted" },
      ] },
      { label: "Write · 3×/day", nodes: [
        { id: "etl", label: "Python job", sub: "12 worker threads", kind: "accent" },
      ] },
      { label: "Store", nodes: [
        { id: "db", label: "Postgres", sub: "one row per source", kind: "accent" },
      ] },
      { label: "Read · per load", nodes: [
        { id: "api", label: "Read API", sub: "…where ok = true", kind: "stop" },
      ] },
      { label: "Client", nodes: [
        { id: "page", label: "Static page", sub: "110 KB · 6 fetches" },
        { id: "digest", label: "Chat digest", sub: "never states revenue", kind: "stop" },
      ] },
    ]}
    edges={[
      { from: "crm", to: "etl" },
      { from: "mkt", to: "etl", label: "each isolated" },
      { from: "etl", to: "db", label: "snapshot + ok" },
      { from: "db", to: "api" },
      { from: "api", to: "page" },
      { from: "db", to: "digest", dashed: true },
    ]}
  />
</Figure>
