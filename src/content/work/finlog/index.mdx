...
The moment an invoice is finalized, that stops. The computed lines are snapshotted into their own table — <code>statement_lines</code> — and never recomputed again, ever, for any reason, because the customer has now seen the number. You can fix the pricing engine the next day and it will not reach backward and silently change what was already sent. If something was wrong, the correction is a new, visible row, not an edit to history.
...
<ArtifactSlot code="A0" name="RegisterToggle" description="The bigger version of the control above — derive and finalize as one state machine, scrubbed live rather than told." />
...
<ArtifactSlot code="A2" name="RateResolver" description="Walk a single API call down the precedence chain live — sandbox, bundle, negotiated rate, catalog, or the unresolved fall-through." />
...
<ArtifactSlot code="A3" name="PricingModels" description="The four curves live — including the slab drop, plotted against the same axis tier is plotted against, at the same threshold." />
...
<ArtifactSlot code="A4" name="SchemaMap" description="The full table map — the effective-dated pattern traced across rates, sandbox state, and client tiers as one recurring shape, not twenty-three separate designs." />

</Chapter>

<Chapter line="006" eyebrow="Two migrations, one lesson" title="Naming as infrastructure" defaultRegister="ledger">

Two migrations in this schema do no computation at all. They exist purely to fix what things are *called*, and they are, without much competition, the best product-management artifacts in the whole codebase — because a vocabulary correction that has to run as a migration is a vocabulary correction that has been taken seriously enough to be treated as infrastructure rather than a Slack message asking everyone to please start saying it differently.

<Marginalia label="Migration 0021">

<code>relabel_graduated_to_tier.sql</code> — before whole-volume pricing existed, "slab" was the only volume model in the system, and it computed what the industry actually calls *graduated* revenue. Once true whole-volume pricing needed to ship, "slab" had to stop meaning graduated first, or the new model would have no correct name available. Every existing row got relabelled slab → tier: revenue-neutral, the arithmetic untouched, one shot, no dry-run needed because nothing about what any invoice owed changed. Only what the model computing it was called.

</Marginalia>

<Marginalia label="Migration 0029">

<code>rename_vocab_audit_notifications.sql</code> — Clients became Accounts, and Accounts became Groups, in every label a user sees. The tables underneath — <code>clients</code>, <code>client_id</code> — were deliberately left alone. Renaming twenty-three tables to chase a label a product manager wanted changed is how you turn a one-afternoon UX fix into a multi-week migration with its own risk of breaking every foreign key reference in the codebase. The migration's own comment notes that update order matters, so the outgoing "account" label never collides with the incoming one mid-flight.
...
The second is sandbox drift, and it's a lifecycle problem rather than a pricing one. Sandbox began as a single bit on the client record: <code>is_sandbox</code>, on or off, for the whole account. The actual request that came in was narrower: an existing *paying* customer wanted to trial one *new* API without being billed for that API specifically, while everything else on their account kept billing normally. A single flag can't express "free for this one thing, billed for everything else," so sandbox became two effective-dated rows per (client, API) pair — <code>is_sandbox = 1</code> from the trial's start, <code>is_sandbox = 0</code> from its go-live date — and billing begins exactly on go-live, with nothing retroactive charged for the trial window. The lifecycle transition isn't a feature bolted onto sandbox. It *is* sandbox, correctly understood.
...
<ArtifactSlot code="A5" name="LeakDetector" description="The three leak classes live against seeded usage — rate gaps priced, sandbox transitions dated, catalog drift resolved or flagged unresolved." />
...
Slab's window-pooling (the same whole-volume mechanic from chapter 004 that reprices an entire month once a threshold is crossed) was being read by one dashboard surface using the *wrong window boundary*, off by a few hours at the edge of a billing period. Not a wrong formula. A boundary condition, in exactly the kind of pricing model complex enough that a boundary condition can hide behind a plausible-looking number for months. The invoice was never wrong — it was never reading that surface. The dashboard was reading its own approximation of the invoice's logic, and the approximation had a seam.
...
<ArtifactSlot code="A6" name="GapRegister" description="All 19 surfaces, their reconciliation status, and the four root causes — including the slab window-pooling gap this chapter describes, at row level." />
...
<span className="type-figure-1" data-money data-rule="double">₹18,67,839.32</span>
...
<ArtifactSlot code="A7" name="YourInvoice" description="A closing statement, built live from this session's own reading — every chapter you actually visited, itemized the way an invoice itemizes a month." />
