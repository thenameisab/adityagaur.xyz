    ...
    <RoutePlate drums="purple-red">
      <div className="container">
        ...
        <Section
          id="vibrant"
          title="Vibrant Mode"
          note="The site's light mode, and a second print run rather than a second palette. Every page that is a page becomes a colour plate; the toggle in the header switches which run you are reading. Every ratio below is recomputed here from src/lib/plates.ts with the site's own WCAG math, and scripts/verify-vibrant.ts asserts the same figures in CI — this table is the published copy, not the source of truth."
        >
          <div className="stack stack--l">
            {/* ── The route plates ── */}
            <div className="stack stack--s">
              <h3 className="type-headline-4 text-primary">
                The seven route plates
              </h3>
              <p className="type-body-3 text-muted">
                An entry page reuses its own subject&rsquo;s pairing from the table
                above, and a redirect keeps the chrome plate, so only seven routes
                load a plate of their own. The floor is {FLOOR}:1 — the site&rsquo;s
                own worst case, set by <code>.theme-dark</code>&rsquo;s{" "}
                <code>--text-faint</code>, rather than WCAG&rsquo;s 4.5.
              </p>
                ...
                <table className={styles.table}>
                  <thead>
                    <tr>
                      ...
                      <th scope="col">Ink credit</th>
                      <th scope="col">Overprint</th>
                      <th scope="col">Ground</th>
                      <th scope="col">Muted</th>
                      <th scope="col">Loud</th>
                      <th scope="col">Slip</th>
                      <th scope="col">Checks</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(Object.keys(ROUTE_PAIRINGS) as RouteDrumsKey[]).map((key) => {
                      const p = ROUTE_PAIRINGS[key];
                      const stock = STOCKS[p.stock].hex;
                      // The four things that could rot independently: the
                      // overprint literal, the slip, the ramp on the INKED
                      // ground (not on bare stock — that is what Vibrant
                      // actually renders), and the loud pass.
                      const ground = mixSrgb(INKS[p.a].hex, stock, 0.12);
                      const muted = mixSrgb(KEY, stock, 0.76);
                      const derived = slipFor(p.slug);
                      const loudMin = p.largeOnly ? 3 : FLOOR;
                      const ok =
                        multiply(INKS[p.a].hex, INKS[p.b].hex) === p.overprint &&
                        derived[0] === p.slip[0] &&
                        derived[1] === p.slip[1] &&
                        passes(p.overprint, ground, FLOOR) &&
                        passes(muted, ground, FLOOR) &&
                        passes(KEY, INKS[p.loud].hex, loudMin);
                      return (
                        <tr key={key}>
                          <th scope="row" className={styles.tokenCell}>
                            <span
                              className={styles.dot}
                              style={{ background: ground, borderColor: KEY }}
                            />
                            <code>{p.route}</code>
                          </th>
                          <td>{creditFor(p)}</td>
                          <td>
                            <code>{p.overprint}</code>{" "}
                            {ratio(p.overprint, ground)}
                          </td>
                          <td>
                            <code>{ground}</code> {ratio(KEY, ground)}
                          </td>
                          <td>{ratio(muted, ground)}</td>
                          <td>
                            {INKS[p.loud].label} {ratio(KEY, INKS[p.loud].hex)}
                            {p.largeOnly ? " — 24px+ only" : ""}
                          </td>
                          <td>
                            {p.slip[0]}, {p.slip[1]}px
                          </td>
                          <td>{ok ? "pass" : "DRIFT"}</td>
                        </tr>
                      );
            ...
            {/* ── Why colour cannot come from type ── */}
            <div className="stack stack--s">
              <h3 className="type-headline-4 text-primary">
                Why the colour is in the ground, not the type
              </h3>
              <p className="type-body-3 text-muted">
                This is the finding that decided the whole theme, and it is the one
                worth reading before changing anything here. An overprint is a
                multiply of two inks, so it is always near-black — which means{" "}
                <code>--text-secondary</code>, the plate&rsquo;s &ldquo;coloured&rdquo;
                type role, is a colour nobody can see is coloured. Law 1 then
                forbids a pure ink from setting small type at all. Between them,
                colour in this system cannot come from type; it can only come from
                area. So Vibrant inks the largest area on the page — the ground —
                and leaves every text role exactly where the plate ramp measured
                it. Three earlier versions put the colour in marks and rules, and
                all three read as black type on cream.
              </p>
            </div>
            ...
            <div className="stack stack--s">
              <h3 className="type-headline-4 text-primary">
                The surface roles, per plate
              </h3>
              <p className="type-body-3 text-muted">
                <code>--bg</code> is drum A at step 1 and carries body copy;{" "}
                <code>--bg-raised</code> is drum B at step 1, so a panel differs
                from the ground by hue rather than value; <code>--bg-card</code> is
                the bare stock, because on a printed sheet the raised thing is the
                part that took no ink. <code>--bg-hover</code> is step 2, which the
                tint ladder licenses for key ink only — so the text ramp collapses
                to key for the duration of a hover, which is why it is not listed
                here with the other roles.
              </p>
              <div className={styles.tableWrap} data-scrollx>
                <table className={styles.table}>
                  <thead>
                  ...
                  <tbody>
                    {(Object.keys(ROUTE_PAIRINGS) as RouteDrumsKey[]).flatMap(
                      (key) => {
                        const p = ROUTE_PAIRINGS[key];
                        const stock = STOCKS[p.stock].hex;
                        const muted = mixSrgb(KEY, stock, 0.76);
                        const faint = mixSrgb(KEY, stock, 0.68);
                        const rows: [string, string][] = [
                          ["--bg", mixSrgb(INKS[p.a].hex, stock, 0.12)],
                          ["--bg-raised", mixSrgb(INKS[p.b].hex, stock, 0.12)],
                          ["--bg-card", stock],
                        ];
                        return rows.map(([role, value], i) => (
                          <tr key={`${key}-${role}`}>
                            <th scope="row" className={styles.tokenCell}>
                              {i === 0 ? <code>{p.route}</code> : null}
                            </th>
                            <td>
                              <span
                                className={styles.dot}
                                style={{ background: value, borderColor: KEY }}
                              />{" "}
                              <code>{role}</code>
                            </td>
                            <td>
                            ...
                            <td>{ratio(KEY, value)}</td>
                            <td>{ratio(p.overprint, value)}</td>
                            <td>{ratio(muted, value)}</td>
                            <td>{ratio(faint, value)}</td>
                          </tr>
                        ));
                      },
                    )}
            ...
            <div className="stack stack--s">
              ...
              <p className="type-body-3 text-muted">
                ...
                be a route plate at all: its overprint{" "}
                <code>#007700</code> sets secondary type at{" "}
                {ratio("#007700", STOCKS.cream.hex)} on cream,{" "}
                {ratio("#007700", STOCKS.grey.hex)} on newsprint and{" "}
                {ratio("#007700", STOCKS.kraft.hex)} on kraft — under the floor on
                every stock the site owns. It was assigned to this page, and the
                check rejected it before it shipped. &ldquo;Text-safe&rdquo; in the
                library&rsquo;s own table means the overprint clears AA on bare
                cream; a plate also has to set that colour on its own tints, which
                is the stricter test and the one that matters.
              </p>
            </div>
          </div>
        </Section>
